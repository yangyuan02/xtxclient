...
(function () {
    ...
    host = window.location.host; //当前域名
    ...
    window.UMEDITOR_CONFIG = {
        ...
        //图片上传配置区
        ,imageUrl: "http://" + host + "/server/api/v1/file/umfile"             //图片上传提交地址
        ,imagePath: 'http://7xnbft.com2.z0.glb.qiniucdn.com/'  //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配置
        ...
        ,toolbar:[
            ...
            'link unlink image formula horizontal',
        ...
        ,initialContent:'<br>'    //初始化编辑器的内容,也可以通过textarea/script给值，看官网例子
        ...
        ,filterRules: function(){
          ...
          var remove_href = function (node) {
            ...
            if (!!node.getAttr('href')) {
              ...
              // 如果是站外链接，则去除链接
              if (node.getAttr('href').indexOf(domain) < 0) {
                node.children[i].setAttr('href', domain);
                node.children[i].setAttr('target', '_blank');
            ...
            // 子元素递归处理方法
            var remove_child_href = function (node) {
              ...
              if (!!node.children.length) {
                ...
                for (var i = 0; i < node.children.length; i++) {
                  ...
                  if (node.children[i].tagName == 'a') {
                    ...
                    // 判断是否有href属性
                    if (node.children[i].getAttr('href')) {

                      // 如果有，则判断链接性质，如果是站外链接，则去除链接
                      if (node.children[i].getAttr('href').indexOf(domain) < 0) {
                        node.children[i].setAttr('href', domain);
                        node.children[i].setAttr('target', '_blank');
                  ...
                  if (node.children[i].children && node.children[i].children.length) {
                    remove_child_href(node.children[i]);
                  }
                }
              }
            }
            remove_child_href(node);
          }
          ...
          return{
            ...
            // 黑名单，以下标签及其子节点都会被过滤掉
            '-': 'script style meta iframe embed object video audio'
          }
      }()
    };
})();
